...
\begin{tikzpicture}[auto,node distance=4cm]
  ...
  \node[attribute] (cpf) [right = 1cm of data-nascimento] {CPF} edge (pessoa);
  ...
  \node[attribute] (id-pessoa) [below = 1.2cm of pessoa] {\underline{Id}} edge (pessoa);
  
  \node[attribute] (login) [below = 1cm of endereco] {Login} edge (pessoa);
  
  \node[attribute] (senha) [below = 1.2cm of cpf] {Senha} edge (pessoa);

  \node[relationship] (rel-pessoa-perfil) [right = 4cm of pessoa] {} edge (pessoa) ;
  
  \node[entity] (perfil) [right = 4cm of rel-pessoa-perfil] {Perfil} edge (rel-pessoa-perfil);

  \node[attribute] (codigo-perfil) [above = 1cm of perfil] {Código perfil} edge (perfil);

  \node[attribute] (id-perfil) [right = 2cm of codigo-perfil] {\underline{Id}} edge (perfil);
...
%  \node[attribute] (visitante) [above right = 2cm of tipo-perfil] {Visitante} edge (tipo-perfil);
 ...
 % \node[attribute] (administrativo) [above = 2cm of tipo-perfil] {Administrativo} edge (tipo-perfil);
  ...
  \node[relationship] (rel-perfil-servico) [below right = 2cm of perfil] {} edge (perfil) ;

  \node[entity] (servico) [below = 2cm of rel-perfil-servico] {Serviço} edge (rel-perfil-servico);

  \node[attribute] (codigo-servico) [above right = 2cm of servico] {Código serviço} edge (servico);
  
  \node[attribute] (descricao) [right = 2cm of servico] {Descrição} edge (servico);

  \node[attribute] (servico-tipo) [below = 1.7cm of servico] {Tipo serviço} edge (servico);

  \node[attribute] (id-servico) [below right = 2cm of servico] {\underline{Id}} edge (servico);
...
%  \node[attribute] (remocao) [below = 1.5cm of servico-tipo] {Remoção} edge (servico-tipo);
  ...
  \path (rel-perfil-servico) edge node [above right] {$1$} (perfil);

  \path (rel-perfil-servico) edge node [right] {$m$} (servico);
  ...
  \node[attribute] (id-docente) [below = 1cm of docente] {\underline{Id}} edge (docente);
  
  \node[attribute] (funcionario-especialidade) [below = 0.8cm of rel-pessoa-perfil] {Especialidade} edge (funcionario);
  
  
  \node[attribute] (funcionario-funcao-tecnica) [left = 0.8cm of rel-perfil-servico] {Função técnica} edge (funcionario);

  \node[attribute] (funcionario-id) [right = 1cm of funcionario] {\underline{Id}} edge (funcionario);
  ...
  \node[attribute] (id-aluno) [above = 0.7cm of aluno] {\underline{Id}} edge (aluno);
  ...
  \node[attribute] (codigo-disciplina) [below = 1.2cm of disciplina] {Código disciplina} edge (disciplina);
  
    \node[attribute] (id-disciplina) [above = 0.7cm of disciplina] {\underline{Id}} edge (disciplina);
  
  \node[attribute] (data-criacao) [above left = 0.8cm of disciplina] {Data criação} edge (disciplina);

  \node[relationship] (rel-oferecimento) [below right = 3.5cm of docente] {} edge (docente) edge (disciplina) edge (aluno);
  \node[attribute] (nota-obtida) [right = 0.5cm of codigo-disciplina] {Nota obtida} edge (rel-oferecimento);
  
  \node[attribute] (data-inicio) [right = 0.5cm of nota-obtida] {Data início} edge (rel-oferecimento);
  
  \node[attribute] (data-fim) [right = 0.5cm of data-inicio] {Data fim} edge (rel-oferecimento);
  
  \path (rel-oferecimento) edge node [above right] {$n$} (docente);

  \path (rel-oferecimento) edge node [above left] {$m$} (disciplina);
